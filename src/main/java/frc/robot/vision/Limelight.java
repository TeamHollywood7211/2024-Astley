...
public class Limelight extends SubsystemBase {
  ...
  public void periodic() {
    if (enable) {
      ...
      LimelightHelpers.Results result =
          LimelightHelpers.getLatestResults(ll).targetingResults; //This what it sees???
      if (result.valid) {
        botpose = LimelightHelpers.getBotPose2d_wpiBlue(ll); //getting robot pose based on limelight data??
          ...
          if (drivetrain.getState().Pose.getTranslation().getDistance(botpose.getTranslation()) < 0.5
            ...
            drivetrain.addVisionMeasurement(
                ...
                Timer.getFPGATimestamp()
                    - (result.latency_capture / 1000.0) //why???
  ...
  public void useLimelight() {
    ...
    if(enable = true)
    {
      NetworkTableInstance.getDefault().getTable(ll).getEntry("ledMode").setNumber(3);
    }
    else
    {
      NetworkTableInstance.getDefault().getTable(ll).getEntry("ledMode").setNumber(1);
    }
